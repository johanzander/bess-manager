...
| `resolve_conflict` | `CONFLICTING`, which also means the PR produced **no CI run at all** — hand to `sweep-prs`, or resolve directly if the diff is ours |
...
| `resume_implementation` | `/implement-issue <n>`, in both shapes this covers, but at very different cost. For a **worktree with no PR** (a session died mid-issue), Step 0 detects the branch and resumes it from the earliest incomplete step — a real session, because the implementation itself is unfinished. For a **PR whose review loop is unfinished** (draft, no terminal verdict yet), Step 0 detects the PR and re-enters directly at Step 11, which invokes `/advance-pr <n>` and exits — cheap, because nothing but the review step is left. Same command, same skill, two different costs; never restart either, the branch commits are the only copy |
