...
- **Scenario Comparison Analysis** — Detailed savings breakdown comparing Grid-only, Solar-only, and Solar+Battery scenarios, showing exactly where savings come from.
- **SolaX inverter support** — Full support for SolaX inverters via homeassistant-solax-modbus integration using VPP active-power commands.
- **Growatt Local Modbus support** — Growatt MIN/MOD/MID (GEN4) and MIX/SPA/SPH (GEN3) inverters via solax_modbus HACS integration with single-segment TOU control (5 entities instead of 45).
- **Fresh install setup wizard** — System starts in unconfigured mode on first install, serving the web UI and setup wizard without crashing. Auto-detects inverter platform, sensors, and pricing provider.
...
- **Nordpool HACS provider support** — Setup wizard handles three pricing providers: `nordpool_official`, `nordpool_hacs`, and `octopus`.
- **Startup progress spinner** — Dashboard shows live progress during initialization instead of 502 Bad Gateway.

### Changed

- **Multi-platform inverter architecture** — Inverter scheduling refactored into `InverterController` base class with platform-specific subclasses (`GrowattMinController`, `GrowattSphController`, `SolaxController`, `SolaxModbusGrowattController`). Platform switching works at runtime without restart.
- **Entity-registry-based discovery** — Sensor discovery uses HA entity registry (immutable unique_id + platform fields) instead of fragile states-based matching. Per-platform suffix maps ensure correct entity resolution.
- **Per-platform sensor storage** — Switching inverter platforms preserves previously entered sensor values.
- **Consistent platform identifiers** — All inverter platform strings follow `<plugin>_<inverter>` convention: `growatt_server_min`, `growatt_server_sph`, `solax_modbus_growatt_min`, `solax_modbus_growatt_sph`, `solax_modbus_native`.
- **Unified energy flow derivation** — `EnergyFlowCalculator` derives load consumption, system production, and self-consumption from 5 core sensors across all platforms.
- **Failure banner coalescing** — Recurring failures produce one banner with occurrence count instead of spamming new entries.

### Fixed

- **Schedule bar showed "Charging from Grid" during solar charging** — Intent classifier now compares dominant energy source (`grid_to_battery > solar_to_battery`) instead of using a near-zero threshold.
- **DP optimizer cycled charge/discharge during solar hours** — Profitability check now accounts for opportunity cost of stored energy; sell price used as cost basis floor when excess solar is available. ([#73](https://github.com/johanzander/bess-manager/issues/73))
- **SOLAR_STORAGE used wrong inverter mode** — Now correctly uses `load_first` instead of `battery_first`, preventing unnecessary grid imports when solar could serve both home and battery.
- **Safari served stale UI after updates** — Added `Cache-Control: no-cache` to `index.html` responses.
- **Settings save button greyed out after sensor changes** — Fixed `stableStringify` to recursively sort keys at all nesting levels.
- **Inverter command failures now visible in dashboard** — Hardware write errors surface as dashboard banners with descriptive operation labels instead of cryptic "Call number.set_value" messages.
- **Grid charge control broken on solax_modbus** — Correctly detects entity domain (`switch` vs `select`) and uses appropriate HA service and state values.
- **MIN inverter 500 errors on price-volatile days** — Hardware writes use content-aware slot assignment when TOU schedule exceeds 9 slots. (thanks [@pookey](https://github.com/pookey))
