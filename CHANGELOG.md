...
- **Grid connection import capacity modeling** *(opt-in, gated on `power_monitoring_enabled`)* — The DP previously had no ceiling on grid import: `home_consumption - home_served` was computed with no cap, so it could plan to leave the battery idle and import an unbounded load through a fuse-limited connection — physically undeliverable, since `HomePowerMonitor` only throttles battery *charging* at runtime, never the household's own load. The optimizer now derives a per-period import energy cap from the same `HomeSettings` the runtime monitor already uses (`voltage × max_fuse_current × safety_margin × phase_count` — each phase is an independent fuse, matching `HomePowerMonitor`'s own balanced-load assumption for battery charging; `HomePowerMonitor` remains the real-time backstop against unbalanced loads) and enforces it as a hard constraint on *total* import (house load + battery grid-charging), not just the charging component. A period whose load alone exceeds the cap forces the battery to discharge and cover the excess rather than being treated as infeasible; if even full discharge can't bring import under the cap, the DP falls back to the minimum-import action available instead of erroring. Grid-charging is throttled (not blocked outright) when serving the load already consumes the cap's headroom — the model-side equivalent of `HomePowerMonitor`'s runtime throttle. The Home settings section is renamed to "Fuse Protection" to reflect that it now drives both day-ahead planning and real-time throttling, and is auto-enabled during setup on a new install once its required sensors are detected. With `power_monitoring_enabled` at its default `False`, behavior is unchanged. Reported via #130's underlying need (a "prevent battery export" request whose real cause was an unmodeled connection ceiling). ([#429](https://github.com/johanzander/bess-manager/issues/429))
