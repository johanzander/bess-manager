...
- **Huawei battery discharge power is no longer reported "Not configured"** — the single signed battery register now backs both the charge and discharge readings, as already done for grid power, and the health panel now shows each direction split by sign instead of the raw negative net value on both. Existing Huawei installs must re-run the setup wizard once to pick up the new mapping. ([#120](https://github.com/johanzander/bess-manager/issues/120))
- **Historical energy backfill no longer misfiles flows on platforms with one signed power register** — where a single entity backs both charge/discharge (Huawei) or import/export (Solis), gap-filling a zero-energy period from InfluxDB credited the whole net value to one direction regardless of sign. ([#120](https://github.com/johanzander/bess-manager/issues/120))
...
- **Phase currents are now always taken from a single device** — a sub-circuit meter (heat pump, EV charger) could previously supply some phases and the grid meter the rest, giving a house current measured at no one point. ([#120](https://github.com/johanzander/bess-manager/issues/120))
