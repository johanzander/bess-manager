...
- **Huawei EMMA / SUN2000 inverter platform (experimental)** — adds source-verified discovery, signed EMMA power monitoring, SUN2000 power/SOC controls, and native 14-period scheduling through `huawei_emma_management.set_tou_periods`. The implementation uses the integration's config-entry ID and does not depend on its Growatt compatibility aliases.
