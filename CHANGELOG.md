...
- **No-solar price arbitrage restored** — On days without solar, the optimizer could miss the overnight-charge → expensive-evening-discharge opportunity and fall back to leaving the battery idle. The optimizer's action set now always includes an explicit "hold" action, so it correctly uses the battery to avoid costly evening grid purchases.
