...
- **DP end-of-horizon terminal value no longer overvalues holding charge on asymmetric buy/sell contracts** — Since the rolling-horizon DP only plans through the end of today, `_calculate_terminal_value`'s "today only" estimate fires on every hourly re-optimization, not just an edge case. It priced a kWh left in the battery at midnight using the median **buy** price, which on contracts where buy and sell diverge significantly (e.g. Belgian dynamic tariffs) produced a terminal value well above any real, achievable evening export price — so the optimizer preferred holding the battery to "collect" that fictitious bonus over exporting at a known, better price available right now. Now based on the median **sell** price, the only value of that kWh that's actually known and realizable at the horizon boundary. ([#244](https://github.com/johanzander/bess-manager/issues/244))
