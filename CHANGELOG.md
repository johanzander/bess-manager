...
- **`HuaweiController.sync_soc_limits` wrote SOC-limit registers unconditionally on every sync instead of read-compare-write** — Unlike every other implemented controller (`GrowattMinController`, `GrowattSphController`, `SolaxController`), Huawei's SOC sync called `set_charge_stop_soc()`/`set_discharge_stop_soc()` on every call regardless of whether hardware already matched configured `max_soc`/`min_soc`, with a docstring rationalizing the unconditional write as harmless. Now mirrors `GrowattMinController.sync_soc_limits`: reads `get_charge_stop_soc()`/`get_discharge_stop_soc()` first, logs "verified" and returns when both match, otherwise writes only the mismatched register(s). Huawei is experimental and has never run in production, so no migration concern — caught during review of #412's `HuaweiEmmaController`, whose override of this method for EMMA turned out to be a fix to the shared Huawei base path rather than EMMA-specific. ([#427](https://github.com/johanzander/bess-manager/issues/427))
