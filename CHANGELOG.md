...
- **Phase currents are now always taken from a single device** — a sub-circuit meter (heat pump, EV charger) could supply some phases and the grid meter the rest, giving a house current measured at no one point. ([#120](https://github.com/johanzander/bess-manager/issues/120))
- **The sensor health panel now shows each direction of a shared signed sensor separately** — Huawei, native SolaX and Solis installs saw the raw net value on both halves, e.g. Charging Power reading −4876 W while the battery discharged. ([#120](https://github.com/johanzander/bess-manager/issues/120))
