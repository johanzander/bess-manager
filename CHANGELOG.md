...
- **The Growatt charge-power-rate register is no longer re-sent to the cloud every cycle when unchanged** — `adjust_charging_power`'s direct-write path (power monitor disabled) now dedupes like the grid-charge and discharge-rate registers already do (#402), so an unchanged charge rate no longer spends a Growatt Open API write toward the daily quota. Field logs showed the un-deduped rate write as the last per-tick churn source after #402/#568, contributing to `GrowattV1ApiError` write failures once the daily quota was exhausted. ([#741](https://github.com/johanzander/bess-manager/issues/741))
