...
- **Growatt VPP Remote Control no longer keeps overriding the inverter after switching away from VPP mode** — switching control mode to TOU, or switching to a different inverter platform entirely, now disables the VPP override automatically. A "Reclaim manual control" action is also available in Settings for installs that got stuck before this fix. ([#479](https://github.com/johanzander/bess-manager/issues/479))
### Fixed
