...
- **Runtime energy collection had no correction for zero-resolution cumulative-counter periods, unlike historical backfill** — cumulative HA counters (e.g. Growatt lifetime discharge energy) only tick in 0.1 kWh steps; when a real discharge is too small to register within a given 15-minute period, its counter delta reads exactly zero and the energy shows up in the following period once the counter finally ticks — a "0 → double" attribution pattern (observed 3 times in one night's debug bundle, periods 5→6, 9→10, 25→26). The historical/backfill collection path already corrects for this by falling back to power (W) sensors when all cumulative-counter flows read zero, but that correction was gated to `is_historical_backfill` only and never applied during runtime (live) collection — the path actually exercised on every 15-minute schedule update. The gap-fill now applies unconditionally; it already degrades gracefully to a no-op when InfluxDB/power sensors aren't configured, so deployments without them are unaffected. Independent investigation of the issue's original hypothesis (that runtime collection wasn't boundary-pinned like the historical path) found runtime collection is already boundary-aligned by construction (`CronTrigger(minute="0,15,30,45")`), so that framing wasn't the root cause — the actual asymmetry was the missing gap-fill. ([#387](https://github.com/johanzander/bess-manager/issues/387))
