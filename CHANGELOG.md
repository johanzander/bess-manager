...
- **Consumption forecast could go stale for up to ~24h during the day, unlike solar which already refreshes every cycle** — `_gather_optimization_data()` (called by the quarterly optimization job, every 15 min) cached the consumption forecast with a plain truthiness check, so once populated at startup it was never refetched until the next restart or the 23:55 `prepare_next_day` job — even if the user changed a home setting (e.g. `default_hourly`) or the underlying source data moved, mid-day. The fix is strategy-aware, not a blanket timer: `sensor` (a live 48h rolling-average HA sensor) and `fixed` now refetch every quarterly cycle, same treatment solar's `controller.get_solar_forecast()` already gets. `influxdb_7d_avg` and `ha_statistics` average a window of full calendar days ending at today's midnight — verified that value is provably unchanged intraday — so they're cached and only refetched once the date rolls over, avoiding unnecessary InfluxDB re-querying. Also removed `self._solar_predictions`, a dead field written at startup/23:55 but never read anywhere — solar was never actually affected by this bug. Verified live against the real backend + mock-HA: after patching `default_hourly` without changing `consumption_strategy` (the only trigger the old cache-invalidation logic had), the very next quarterly tick picked up the new value with no restart. ([#395](https://github.com/johanzander/bess-manager/issues/395))
