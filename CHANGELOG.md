...
- **Grid connection import capacity modeling** *(opt-in, gated on `power_monitoring_enabled`)* — The DP previously had no ceiling on grid import: `home_consumption - home_served` was computed with no cap, so it could plan to leave the battery idle and import an unbounded load through a fuse-limited connection — physically undeliverable, since `HomePowerMonitor` only throttles battery *charging* at runtime, never the household's own load. The optimizer now derives a per-period import energy cap from the same `HomeSettings` the runtime monitor already uses (`voltage × max_fuse_current × safety_margin` — deliberately not multiplied by phase count, matching `HomePowerMonitor`'s own worst-phase gating for unbalanced 3-phase loads) and enforces it as a hard constraint on *total* import (house load + battery grid-charging), not just the charging component. A period whose load alone exceeds the cap forces the battery to discharge and cover the excess rather than being treated as infeasible; if even full discharge can't bring import under the cap, the DP falls back to the minimum-import action available instead of erroring. Grid-charging is throttled (not blocked outright) when serving the load already consumes the cap's headroom — the model-side equivalent of `HomePowerMonitor`'s runtime throttle. With `power_monitoring_enabled` at its default `False`, behavior is unchanged. Reported via #130's underlying need (a "prevent battery export" request whose real cause was an unmodeled connection ceiling). ([#429](https://github.com/johanzander/bess-manager/issues/429))
...
- The DP optimizer now models grid connection import capacity, gated on `power_monitoring_enabled`. ([#429](https://github.com/johanzander/bess-manager/issues/429))
