...
**PR #461's MILP has a residual self-throttle export-credit bug beyond the one already fixed in that branch's `0df1d8bc`.** That commit added `throttled=0 ⟹ credited_exp == exp` (forcing full credit when genuinely not self-throttled) but never added the converse constraint (`exp <= threshold ⟹ throttled=1`, forcing zero credit when the export genuinely IS below the ~0.01 kWh hardware self-throttle threshold). Without the converse, `throttled` stays a free binary whenever `exp` is small — the solver can always choose `throttled=0` and claim full revenue credit for exports that should have been zero-credited. Measured on `regression_2026_08_02_043728.json`: 11 DISCHARGE periods export ≤ 0.01 kWh and are fully credited, worth exactly 0.031857 SEK — which is almost the entire gap between the MILP's reported optimum (-6.012542) and the true DP-reward-objective optimum reachable by a full-horizon exact solve (-5.984652). This is separate from and additional to the four MILP bugs already documented in PR #461's own description (import/export exclusivity, AC-cap sentinel, the credited-exp lower-bound fix in `0df1d8bc`, and the shadow-price LP-dual non-uniqueness). Confirmed structurally impossible in the grid-DP/PWL approach, since self-throttle there is a deterministic formula (`if exp <= threshold: exp = 0`) with no LP slack variable to exploit — this bug class is intrinsic to the MILP's constraint-encoding paradigm, not something a future MILP fix could accidentally miss again in the same code path (it needs the converse constraint added).
