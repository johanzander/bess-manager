...
**`_idle_battery_flows`'s below-floor guard now zeroes real energy, not just floor artefacts** (`core/bess/dp_battery_algorithm.py`): its comment justifying `if soe < min_soe_kwh: return 0.0, 0.0` said the delta was "a floor artefact, not solar production" — true before this PR's fix, since `_state_transition` always fabricated a jump to `min_soe_kwh`. After the fix, IDLE starting below the floor with a genuine solar surplus can legitimately raise `next_soe` (real stored energy), but this guard still zeroes `battery_charged` unconditionally, while `_build_period_data`'s separate `energy_stored = next_soe - soe` (used for wear cost/cost basis) has no matching guard — producing an internally inconsistent period (energy vanishes from the reported ledger while wear cost still reflects it). Pre-existing pattern, not introduced by #233's fix, but that fix makes this exact scenario reachable in a way worth a follow-up.

**`_interpolate_value`'s index clamp flattens continuation value below the SOE floor**: `V`'s grid (`soe_levels`) starts exactly at `min_soe_kwh`, so any `next_soe` below the floor clamps to `V_row[0]` regardless of how far below floor it is — candidates ending at different below-floor SoEs get identical continuation credit. Bounded in practice (discharge is already excluded below floor, and `next_soe` is monotonically non-decreasing once below floor), but worth a comment noting the approximation if this file is touched again.
