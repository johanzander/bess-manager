...
def test_system_with_energy_manager(mock_controller):
    ...
    system._price_manager.source = MockSource([0.5] * 24)  # noqa: SLF001

    # Test energy manager initialization
    assert system._energy_manager is not None, "Energy manager should be initialized"  # noqa: SLF001

    # Test consumption predictions
    assert hasattr(
        system._energy_manager,
        "get_consumption_predictions",  # noqa: SLF001
...
def test_energy_balance_with_solar(mock_controller):
    """Test energy balance tracking with solar generation."""

    # First, ensure the mock controller has required methods
    if not hasattr(mock_controller, "get_sensor_value"):
        # Add the method to the mock controller
        def mock_get_sensor_value(sensor_name):
    ...
    if not hasattr(em, "_system_production"):
        em._system_production = {}  # noqa: SLF001

    for hour in range(6, 18):
        em._system_production[hour] = 2.0  # Add some solar production  # noqa: SLF001
