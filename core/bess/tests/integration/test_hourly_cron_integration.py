...
class MockDailyViewBuilder:
    ...
    def build_daily_view(self, current_hour, buy_price, sell_price=None):
        ...
        for hour in range(24):
            price = (
                buy_price[hour]
                if isinstance(buy_price, list) and hour < len(buy_price)
                else 1.0
            )
            hourly_data.append(
                HourlyData(
        ...
        return DailyView(
            ...
            actual_savings_so_far=0,
            predicted_remaining_savings=0,
            actual_hours_count=0,
            predicted_hours_count=24,
...
class MockGrowattController:
    ...
    def set_inverter_time_segment(
        ...
        logger.info(
            f"[MOCK] Set TOU segment {segment_id}: {start_time}-{end_time} ({batt_mode})"
        )
...
def simple_system(test_prices):
    ...
    system.sensor_collector = MockSensorCollector(
        controller, system.battery_settings.total_capacity
    )
    system.historical_store = MockHistoricalDataStore(
        system.battery_settings.total_capacity
    )
    system.daily_view_builder = MockDailyViewBuilder(
        None, None, system.battery_settings.total_capacity, 0.4
    )
...
def simulate_hourly_cron_job(system, controller, hour):
    ...
    controller.current_settings["grid_charge"] = not controller.current_settings[
        "grid_charge"
    ]
    controller.current_settings["discharge_rate"] = (
        100 if controller.current_settings["discharge_rate"] < 50 else 0
    )
...
def test_hourly_updates(simple_system):
    ...
    for hour in test_hours:
        ...
        assert (
            grid_calls_after > grid_calls_before
        ), f"Should make grid calls for hour {hour}"
        assert (
            discharge_calls_after > discharge_calls_before
        ), f"Should make discharge calls for hour {hour}"
...
def test_full_day_simulation(simple_system):
    ...
    for decision in hourly_decisions:
        logger.info(
            f"{decision['hour']:4d} | {decision['grid_charge']!s:11} | {decision['discharge_rate']:9d}"
        )
