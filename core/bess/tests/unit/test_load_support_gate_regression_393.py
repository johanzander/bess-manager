...
plan-scaled baseline regardless of shadow_price" (#393). It now pins the
...
pacing protects is already inside the gate's own comparison
(`buy_price * eff_d >= shadow_price`). Gate closed -> import, reservation
protected by construction. Gate open -> the energy is worth more used now than
saved, so there is nothing being reserved. The gate does not override
...
pacing-override, and is still reproduced below as a sanity check on the
fixture. The breadth is real and corpus-wide (the gate is open in 431 of 603
LOAD_SUPPORT periods, 71.5%, and raises the ceiling in 427 of them); it is
...
period's real (buy_price, shadow_price, planned action) triple is replayed
...
# so the buy price the production gate sees is exactly the fixture's own buy
# price -- the same number the DP priced its shadow prices against.
...
def _make_bsm(
    ...
    """A BSM running the fixture's real battery (rate limits and efficiency
    drive both the plan-scaled baseline and the gate) at the given buy price."""
...
def _apply_load_support_period(
    bsm: BatterySystemManager, action_kwh: float, shadow_price: float
) -> None:
    intents = ["IDLE"] * 96
    ...
    period_data = PeriodData(
        ...
        decision=DecisionData(
            strategic_intent="LOAD_SUPPORT", shadow_price=shadow_price
        ),
    )
...
def _load_support_periods() -> tuple[list[tuple[int, float, float, float]], dict]:
    """Replay the real fixture through the DP and return, per LOAD_SUPPORT
    period, (index, buy_price, shadow_price, planned battery action kWh)."""
    inp = _scenario_inputs(_load_fixture())
    result = optimize_battery_schedule(**inp)
    buy_price = inp["buy_price"]
    periods = [
        (i, buy_price[i], pd.decision.shadow_price, pd.decision.battery_action or 0.0)
...
def test_gate_condition_is_favorable_for_most_load_support_periods():
    """Sanity check on the fixture itself: reproduces #393's empirical finding
    (51/67, ~76% gate-open). If this stops being true (e.g. the fixture or DP
    changes), the test below is no longer exercising the regime it guards --
    investigate before touching the assertions."""
    periods, inp = _load_support_periods()
    eff_d = inp["battery_settings"].efficiency_discharge

    gate_open_count = sum(
        1
        for _, buy, shadow, _ in periods
        if intra_period_discharge_gate(buy, shadow, eff_d) == 100
    )
...
def test_load_support_ceiling_follows_the_gate_on_real_data():
    ...
    eff_d = settings.efficiency_discharge

    opened = 0
    held = 0
    for i, buy, shadow, action_kwh in periods:
        action_kw = action_kwh / dt
        baseline = min(
            100, max(0, round(abs(action_kw) / settings.max_discharge_power_kw * 100))
        )
        gate = intra_period_discharge_gate(buy, shadow, eff_d)

        bsm, controller = _make_bsm(buy, settings)
        _apply_load_support_period(bsm, action_kwh, shadow)
        ...
        assert applied == max(baseline, gate), (
            ...
            f"(buy={buy:.4f}, shadow={shadow:.4f}) -- LOAD_SUPPORT must "
            "follow the shadow-price gate (#520)"
        )
        if gate == 100:
            opened += 1
        else:
