...
def test_check_health_unconfigured_sensors(unconfigured_controller):
    monitor = HomePowerMonitor(unconfigured_controller, HomeSettings(), BatterySettings())
    ...
    for check in health[0]["checks"]:
        if check["status"] == "ERROR":
            assert (
                "No entity ID configured" in (check["error"] or "")
                or "Failed to calculate available charging power" in (check["error"] or "")
            )
