...
7. Before comparing plans, check whether the inverter still holds the segments this controller programmed (`needs_hardware_reconciliation`, default no-op; only the differential-update platform implements it). The comparison in step 6 is plan-against-plan, so once a segment is written and the plan stops changing, nothing would look at the inverter again — a write it silently dropped would run wrong until the window ended. This is also what re-attempts a write that vanished without raising; one that raises is retried through `_hardware_write_pending`
