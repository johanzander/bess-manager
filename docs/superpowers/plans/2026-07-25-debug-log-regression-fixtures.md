...
Expected: PASS (physical constraints hold, savings positive — the generic invariants this bug didn't violate, confirming the new fixture integrates cleanly with the existing test infrastructure with zero additional wiring).

- [ ] **Step 5: Run the full fast + slow suites**
...
Expected: all pass, no regressions, count increased by exactly the new fixture's parametrized case plus Tasks 1-3's new tests.

- [ ] **Step 6: Commit**

```bash
git add core/bess/tests/unit/data/regression_2026_07_25_090230.json
git commit -m "test: add issue #269 regression fixture, generated from the real debug bundle

Dogfoods the new --issue flag against the actual bundle used to diagnose
and verify PR #391, proving the generator -> core/bess/tests/unit/data/ ->
test_scenarios.py chain works end-to-end on real data."
```
