...
for t in range(T):
    ...
    MS3 = RATE_T / 1.0
    con(
        [(s2b[t], 1), (stS[t], -1 / ETA_C), (sig[t], -MS3), (S[t], -MS3)],
        -2 * MS3,
        np.inf,
        "s2b_force",
    )
    ...
    con(
        [(stI[t], 1), (iot[t], MI1), (I_[t], -MI1)],
        c_idle[t] - MI1,
        np.inf,
        "idle_force",
    )
    con(
        [(stI[t], 1), (E[t], 1), (iot[t], -MC2), (I_[t], -MC2)],
        EMAX - 2 * MC2,
        np.inf,
        "idle_force",
    )
    ...
    con(
        [(k[t], DIS_STEP)],
        -np.inf,
        ac_headroom[t] if ac_headroom[t] < RATE_T * 4 else np.inf,
    )
...
res = milp(
    ...
    bounds=(
        (lb, ub)
        if False
        else __import__("scipy.optimize", fromlist=["Bounds"]).Bounds(lb, ub)
    ),
